...
kubectl delete secret jenkins-admin dockerhub-credentials github-ci-token \
dockerhub-config -n jenkins --ignore-not-found
...
### 6. Esperar y mostrar credenciales #########################################
kubectl rollout status sts/jenkins-local-k3d -n jenkins --timeout=600s
echo -e "\n✅ Jenkins listo → http://localhost:8080"
echo   "👤 Usuario: $JENKINS_ADMIN_USER"
echo   "🔑 Contraseña (texto plano en .env): $JENKINS_ADMIN_PASSWORD"
